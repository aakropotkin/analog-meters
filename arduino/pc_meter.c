...
const float RED_ZONE_PERC  = .80;  /* Percent at which LED goes from green
                                      to red */
const long  SERIAL_TIMEOUT = 3000; /* How long to wait until serial
                                      "times out" */
const int   CPU_READINGS   = 4;    /* Number of readings to avg. for
                                      CPU load% */
...
int           cpuIndex = 0;              /* current position in array for
                                            cpu load% */
...
loop()
{
  ...
  while ( Serial.available() > 0 )
    {
      ...
      /*
       * Thought this might be needed based on example I studied, seems
       * okay without
       */
      /* buffer[4] = '\0'; */
      ...
      switch ( buffer[0] )
        ...
        case 'C':
          ...
          /* Advance index */
          cpuIndex = cpuIndex + 1;
          if ( cpuIndex >= CPU_READINGS ) cpuIndex = 0;
          break;
  ...
  if ( currentMillis - lastSerialRecd > SERIAL_TIMEOUT ) screenSaver();
}
  ...
  while ( Serial.available() == 0 )
    {
      ...
      if ( aPos == 100 ) incAmt = -1;
      else if ( aPos == 0 ) incAmt = 1;
...
initCpuValues()
{
  for ( int counter = 0; counter < CPU_READINGS;
        counter++ ) cpuReadings[counter] = 0;
  cpuTotal = 0;
}
